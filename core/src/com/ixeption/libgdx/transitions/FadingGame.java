...
public class FadingGame extends Game {
	...
	public void create() {
		...
		this.nextScreenFBO = new FrameBuffer(Pixmap.Format.RGBA8888, Gdx.graphics.getWidth(), Gdx.graphics.getHeight(),
				false);
		...
		if (nextScreen == null) {
			...
			if (transitionRunning && currentTransitionTime >= transitionDuration) {
				...
				if (screenTransition != null) {
					...
					screenTransition.render(batch, currentScreenFBO.getColorBufferTexture(),
							nextScreenFBO.getColorBufferTexture(), percent);
	...
	public void resize(int width, int height) {
		...
		if (currentScreenFBO != null) {
			this.currentScreenFBO.dispose();
		}
		if (nextScreenFBO != null) {
			this.nextScreenFBO.dispose();
		}
	...
	/**
	 * Sets the {@link ScreenTransition} which is used. May be {@code null} to
	 * use instant switching.
	 * 
